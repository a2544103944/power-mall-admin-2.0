<template>
  <el-popover width="370" placement="right" trigger="click">
    <div>
      <el-input v-model="specName" ref="InpuSpecRef" @keyup.enter="addSpecName()" placeholder="请输入规格名称">
        <template #append>
          <el-button type="primary" size="small" @click="addSpecName()">新增</el-button>
        </template>
      </el-input>
    </div>
    <template #reference>
      ...
      <div v-for="(item, index) in specData.specList" :key="index">
        ...
        <div style="display: flex;">
          <div v-for="(tagNmae, tagIndex) in item.tags" :key="tagIndex">
            <el-tag @close="onDeletSpecValue(index, tagIndex)" closable style="margin: 0 10px;">{{ tagNmae }}</el-tag>
          </div>
          <div>
